...
public abstract class MinDistanceButLastPLG implements PointListGenerator {

  private final Point2D.Double start;
  private final double minDistance;
  private final List<Point2D.Double> toAvoid;

  public MinDistanceButLastPLG(Point2D.Double start, double minDistance) {
    this.start = start;
  ...
  private boolean proposedIsInContentionWithAnother(Point2D.Double p, List<Point2D.Double> points) {
    return toAvoid.stream().filter(o -> o != toAvoid.get(toAvoid.size() - 1))
        .mapToDouble(o -> o.distance(p)).min().orElse(Double.MAX_VALUE) < minDistance;
  }
}
